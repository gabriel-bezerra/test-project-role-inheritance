...
function create_subproject() {
  ...
  curl -H "X-Auth-Token: $TOKEN" -H "Content-type: application/json" \
       -d '{"project": {"description": "--optional--", "domain_id": "'"$DOMAIN_ID"'", "enabled": true, "name": "'"$PROJECT_NAME"'", "parent_project_id": "'"$PARENT_PROJECT_ID"'"}}' \
       http://$KEYSTONE_HOST:5000/v3/projects \
    | ./jq '.project.id' -r
}
...
function delete_project() {
  ...
  curl -X DELETE -H "X-Auth-Token: $TOKEN" "http://$KEYSTONE_HOST:5000/v3/projects/$PROJECT_ID"
}
...
function add_inherited_project_role() {
  local TOKEN=$1
  local PROJECT_ID=$2
  local USER_ID=$3
  local ROLE_ID=$4
